...
## Track B handoff for the next session (14 Sep, ~02:00 IST)

Read this section, then README.md "Where things stand", REQUESTS.md (the two open boxes from B) and DEMO.md sections 4 to 6. The previous Track B session ended at context limit with a clean tree; every Track B change is committed.

**State.** 139 tests, 29/29 scenarios, 27/27 matrix cells, 12/12 mutants, all green on twins. `scorecard.html` renders from `evals/results/final.json`, `baseline.json`, `mutants.json`, the live dry-run trace `traces/live-dry.jsonl` (falls back to `traces/demo.jsonl`) and the two model-comparison traces; the third column (`compare-anthropic`) is silently skipped until it exists. `BRIEF.md` and `VIDEO.md` are written for the twin demo; the brief's limitation paragraph and the scorecard's limitation box both say live GitHub and Slack were exercised read-only.

**Sandbox.** GitHub org, leaver login, Slack workspace and leaver email are in `.env` and `hr.json` (gitignored). Never write them into a tracked file; the Operator asked for this twice. Seeded: three repos (billing with a write collaborator and a read-write deploy key referenced from `.github/workflows/deploy-staging.yml`, legacy-billing with the injected description, infra with a read collaborator and an idle read-only key), plus one extra test repo; Slack channels including one with the injected topic, a private `it-offboarding` with the bot, and the general channel. Both deploy keys are titled `<leaver login>-laptop` and `<leaver login>-old-laptop` because live GitHub attributes keys by `added_by`. The leaver's GitHub profile name equals the HR name so identity gets two signals.

**Verified live so far, all read-only.** `python3 -m evals.live_smoke --app github` and `--app slack` pass. `python3 -m evals.live_run --user <leaver> --hr hr.json --apps github,slack --dry-run --trace traces/live-dry.jsonl` passes: 47 reads, 0 writes, both identities on two signals, in-use key escalated (R2), idle key `revoke`, both injections logged (F7), deactivation escalated (R9), every write `skipped_dry_run` with a diff. Also recorded once into `cassettes/dry.json`. Nothing has been applied, so nothing needs restoring before the next run.

**`.env` now has every key filled**, including `GOOGLE_CLIENT_ID/SECRET/REFRESH_TOKEN` and `GOOGLE_SHEET_ID` (the Operator got an OAuth client from a friend). Neither `google-api-python-client` nor `anthropic` is installed on this machine (Python 3.14, no venv). Consequence: `cli.py` now auto-selects all four apps in live mode because `GOOGLE_REFRESH_TOKEN` is set, so pass `--apps github,slack` (or `github,slack,sheets` after `pip install google-api-python-client google-auth`) explicitly. Live Drive is skipped on purpose: consumer-Gmail ownership transfer needs the owner's own credentials, so trap T1 cannot run live, and the cut list allows dropping live Drive. Track A's latest commit escalates an unreadable app (`unavailable:<app>`, F6 finding) instead of aborting, so a Drive 403 no longer kills a run either way.

**Open, in order.**

1. Track A: escalate `#general` channel items via `hints.is_general` (REQUESTS.md, 11:30 PT). Until merged, do not run a real apply; the first write is that kick and Slack refuses it. When the Operator says A has pushed, they fetch, you `git merge --ff-only origin/main` (or stash the shared files and rebase), rerun the unit tests and the dry run, and check the console (`python3 -m report.console --trace traces/live-dry.jsonl --out console.html`) shows the general channel as escalated.
2. Then, with a "go" per command and every irreversible write named out loud before it: `python3 cli.py plan --mode live --user <leaver> --hr hr.json --apps github,slack --out plan.json --trace traces/run.jsonl`, edit `plan.json`, `python3 cli.py apply --plan plan.json --mode live --hr hr.json --apps github,slack --record cassettes/demo.json --trace traces/run.jsonl`. `--model anthropic` only if the Operator agrees to spend credits; otherwise `heuristic`, and say so in the brief. Confirm on GitHub that the in-use key still exists and the idle one is gone; screenshot for the brief. Then `cp traces/run.jsonl traces/demo.jsonl`, `make scorecard`, `make console`, and re-derive the step ids in `VIDEO.md` from the new trace.
3. Third model column: `pip install anthropic`, ask about credits, `python3 -m evals.runner --only h1_full_run --model anthropic --label compare-anthropic`, `make scorecard`.
4. Optional live Sheets: install the Google packages, `--apps github,slack,sheets`, `--sheet $GOOGLE_SHEET_ID`. The sheet's header row is `run_id, step, app, op, resource, status, diff, undo`.
5. Update the limitation text in DECISIONS.md and the limitation paragraph in BRIEF.md to match whatever actually ran; the scorecard reads the former verbatim.

**Rules that still hold.** Never commit; hand over a `B:` message with the Co-Authored-By line and the Operator commits. Never edit Track A files unless told to in that message. Never call a live API without a per-command go, and never anything outside the sandbox org and workspace. Ask before anything that spends `MODEL_API_KEY`. Keep real names out of tracked files.
