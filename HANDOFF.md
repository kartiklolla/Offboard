...
**`evals/mutants.py`** answers the green baseline. Eleven mutants, each a context manager that monkeypatches one defence out of the running agent (two-signal identity, R1 to R5, phase ordering, stop-on-failure, the read-back, the page-count check, `verify_summary`, and the whole policy veto), runs all 28 scenarios, and records which went red per class. On the merged tree all 11 are caught, each by the class that claims to prove it. `python -m evals.mutants` writes `evals/results/mutants.json`; the scorecard renders it with `--mutants` as a mutant-by-class grid where a red cell is a defence the suite would not notice losing. Track A's files are never edited; the patches are restored on exit and `tests/test_mutants.py` proves it.
...
Not started: B5 live and B7 the cassette, both waiting on sandbox facts, `.env`, and an explicit go per command. The current full command for the scorecard is `python -m report.scorecard --results evals/results/final.json --baseline evals/results/baseline.json --mutants evals/results/mutants.json --trace traces/demo.jsonl --compare a.jsonl b.jsonl c.jsonl --out scorecard.html`.
